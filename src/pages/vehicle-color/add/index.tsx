...
import type { VehicleColorCrudContentProps, VehicleColorCrudContainerProps, VehicleColorCrudContentForm } from './index.model';

const VehicleColorCrudContainer = ( props: VehicleColorCrudContainerProps ): ReactElement => {
  /** Hook section */

    const Router = useRouter();
    const [ mode, setMode ] = useState<CrudMode>('add');
    const [ antdForm ] = Form.useForm<VehicleColorCrudContentForm>();
    ...
    useEffect(() => {
        if(Router.isReady) {
            switch(Router.route){
                case '/vehicle-color/add': {
                    setMode('add'); 
                    break
                };
                case '/vehicle-color/edit/[id]': {
                    setMode('edit'); 
                    antdForm.setFieldsValue({});
                    setIdFromPath();
                    break
                };
                case '/vehicle-color/view/[id]': {
                    setMode('view'); 
                    setIdFromPath();
                    break;
                }
    ...
    const onSubmitForm = (value: VehicleColorCrudContentForm) =>{
    ...
    const contentProps: VehicleColorCrudContentProps = {
        mode,
        onSubmitForm,
        onClickBack
    };
