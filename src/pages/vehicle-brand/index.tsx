 ...
 */
import type { VehicleBrandContentProps, VehicleBrandContainerProps, FormValues,GetDataParams, FormSearch } from './index.model';
...
import { carBrandApi } from '@/services/central-api/index';
import {   CtlMstCarBrand } from '@/services/central-api/generated';
...
const VehicleBrandListContainer: NextPageWithLayout<VehicleBrandContainerProps> = ( props: VehicleBrandContainerProps ): ReactElement => {
  /** Hook section */
  const fetchGetData = useApi(carBrandApi,carBrandApi.apiMasterCarBrandGetGet)
  const fetchSaveData = useApi(carBrandApi,carBrandApi.apiMasterCarBrandSavePost)
  const fetchRemoveData = useApi(carBrandApi,carBrandApi.apiMasterCarBrandDeleteDelete)
  ...
  const [ dataList, setDataList ] = useState<CtlMstCarBrand[]|undefined>();
  ...
  const getData = async ({
    ...
    try {
      const res = await fetchGetData.fetch({//carBrandApi.apiMasterCarBrandGetGet({
  ...
  const handleCloseModal = () => {
    ...
    const onClickToEdit = async (param: {id: string}) => {
      ...
      try {
        ...
        if (res && res?.items && res?.items?.length>0 ) {
          ...
          form.setFieldsValue({
            carBrand: responseData.brandName || '' , 
    ...
    const onSaveFormModal = (): void => {
      ...
      form
        ...
        .then((values) => {
          ...
          fetchSaveData.fetch({masterCarBrandInput :paramForSave}).then(()=>{
                getData({
                  currentPage: page,
                  currentPageSize: pageSize,
            ...
            setModalProps({
              ...
              onConfirm: () => {
                ...
                fetchSaveData.fetch({masterCarBrandInput:paramForEdit}).then(() => {
   ...
   if (extra.action==='sort') {
    ...
    const contentProps: VehicleBrandContentProps = {
        page,
        pageSize,
        dataList,
        isAllChecked,
                          ...
                          <Row gutter={[16, 16]} justify="start"> 
                            ...
                            <Col xs={24} md={10}> 
                              <Form.Item
                                ...
                                wrapperCol={{ span: 16 }} 
                              >
                                <Input className="w-full z-10	"  placeholder="ค้นหายี่ห้อรถยนต์" allowClear />
                              </Form.Item>
...
const renderOptions = (props: VehicleBrandContentProps): ReactElement => {
  return (
    <>
...
const renderTable = (props: VehicleBrandContentProps): ReactElement => {
...
const renderModalForm = (props: VehicleBrandContentProps): ReactElement => {
              ...
              >
                  <Form form={props.form} layout="vertical" name="modal_form_layout">
                    <Row gutter={[16, 5]}>
                      ...
                      <Form.Item
                          name="carBrand"
                          label="ชื่อยี่ห้อรถยนต์"
                          rules={[{ required: true, message: 'กรุณากรอกข้อมูล!' }]}
                        >
                          <Input placeholder="ชื่อยี่ห้อรถยนต์" allowClear />
                        </Form.Item>
                      </Col>
...
export default VehicleBrandListContainer;