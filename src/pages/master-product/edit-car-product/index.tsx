...
import { vehicleTypeApi  , vehicleProductApi, carBrandApi, vehicleModelApi, vehicleItemApi} from '@/services/central-api/index';
import {   CtlMstVehicleType, VehicleItemSearchOutput, VehicleProductOutput} from '@/services/central-api/generated';
...
const MasterProductCrudContainer: NextPageWithLayout<MasterProductCrudContainerProps> = ( props: MasterProductCrudContainerProps ): ReactElement => {
  ...
  const fetchGetData = useApi(vehicleProductApi,vehicleProductApi.apiVehicleProductGetGet)
  const fetchSaveData = useApi(vehicleProductApi,vehicleProductApi.apiVehicleProductSavePost)
  // const fetchRemoveData = useApi(vehicleProductApi,vehicleProductApi.)
  const { selectedRowKeysList, setSelectedRowKeysList , handleRowSelectionOne, handleRowSelectionAll } = useRowSelection<VehicleItemSearchOutput>();
  ...
  const [ dataList, setDataList ] = useState<VehicleProductOutput[]|undefined>();
  ...
  const setDefaultSelectedItem =async () =>{
        ...
        const resGetVehicleItem = await  fetchGetDataVehicleItem.fetch({
          pageIndex: 1,
          pageSize: 1000,
          tenantCode,
          branchCode,
        ...
        const ids2 = selectedVehicleItemProduct?.map(item => Number(item.vehicleItemId)); // Ensure ids are numbers
          // Find common IDs
        const sameIDList: number[] = _.intersection(ids1, ids2).filter(Boolean); 
  ...
  const handleCloseModal = () => {
    ...
    const onSaveFormModal = async () =>{
        ...
        const paramForEdit = {
          "productId": productId,
          "itemId": numberSelectedRowKeysList,
          "tenantCode": tenantCode,
          "branchCode": branchCode
        }
        ...
        setModalProps({
          ...
          onConfirm: async () => {
            ...
            setIsLoadingPage(true);
            fetchSaveData.fetch({vehicleProductInput :paramForEdit}).then(() => {
                  getData({
                    currentPage: page,
                    currentPageSize: pageSize,
                    isLoadingTable:false
    ...
    const contentProps: MasterProductCrudContentProps = {
        ...
        vehicleItemList:fetchGetDataVehicleItem.data?.items || undefined,
        formSearchManageItem,
  ...
  return (
    <>
      <AntTable
        ...
        onChange={props.onHandleTableItemChange}
        showSorterTooltip={false}
        loading={props.isTableLoading}
        // style={{boxShadow: 'rgba(0, 0, 0, 0.24) 0px 3px 8px'}}
        pagination={{
          current: props.pageItemTable,
          pageSize: props.pageSizeItemTable,
          total: props.totalDataItemTable,
          pageSizeOptions: [10, 15, 20, 50],
          onChange: props.onChangePageTableItem,
          showSizeChanger: true,
          simple: true,
          showTotal: (total: number, range: [number, number]) => onShowTotal(total, range),
        }}
