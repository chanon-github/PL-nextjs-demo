 ...
 */
import type { MasterProductContentProps, MasterProductContainerProps, FormValues ,FormSearch,GetDataParams } from './index.model';
/**
 *  Copyright (C) 2021, 2Smooth Digital Co. Ltd., all rights reserved
...
import {  productApi} from '@/services/central-api/index';
...
const MasterProductListContainer: NextPageWithLayout<MasterProductContainerProps> = ( props: MasterProductContainerProps ): ReactElement => {
  /** Hook section */
  const cookies = new Cookies();

  const fetchGetData = useApi(productApi,productApi.apiProductGetGet)
  const fetchSaveData = useApi(productApi,productApi.apiProductSavePost)
  const fetchRemoveData = useApi(productApi,productApi.apiProductDeleteDelete)
  const { selectedRowKeysList, setSelectedRowKeysList , handleRowSelectionOne, handleRowSelectionAll } = useRowSelection<CtlMstVehicleType>();
  ...
  useEffect(() => {
    ...
    if (productId) {
      cookies.remove('productId');  // Correct method for removing a cookie
    }
    if (productName) {
      cookies.remove('productName');  // Correct method for removing a cookie
    }
  ...
  const handleCloseModal = () => {
    ...
    const onClickToEdit = async (param: {id: number}) => {
      ...
      try {
        ...
        if (res && res?.items && res?.items?.length>0 ) {
          ...
          form.setFieldsValue({
            productName: responseData.productName || undefined  , 
    ...
    const onClickToManageCarProduct = (param: {id: number,name:string}) => {

          cookies.set('productId', param?.id?.toString(), { sameSite: 'strict' });  
          cookies.set('productName', param.name, { sameSite: 'strict' });
          Router.push(`/master-product/edit-car-product`);

    }

    const onClickToManageCarStock = (param: {id: number,name:string}) => {

      cookies.set('productId', param?.id?.toString(), { sameSite: 'strict' });  
      cookies.set('productName', param.name, { sameSite: 'strict' });
      Router.push(`/master-product/edit-car-stock`);
    ...
    const onSubmitSearch = () =>{
      ...
      .then((values) =>{
        ...
        getData({
          currentPage: page,
          currentPageSize: pageSize,
          keywordSearch:values.productName,
      ...
      form
        ...
        .then((values) => {
          ...
          const paramForSave = {
            ...
            // "tenantCode": "0002",
            // "branchCode": "00002"
          fetchSaveData.fetch({productInput:paramForSave}).then(()=>{
            ...
            const code = dataIdForEdit ? Number(dataIdForEdit):null;
            const paramForEdit = {
                "id": code,
                "productName": values.productName,
                "status": values.status,
            ...
            setModalProps({
              ...
              onConfirm: () => {
                ...
                fetchSaveData.fetch({productInput:paramForEdit}).then(() => {
                  getData({
                    currentPage: page,
                    currentPageSize: pageSize,
    ...
    const contentProps: MasterProductContentProps = {
        page,
        pageSize,
        dataList,
        totalData,
                          ...
                          <Row gutter={[16, 16]} justify="start"> 
                            ...
                            <Col xs={24} md={10}> 
                              <Form.Item
                                ...
                                wrapperCol={{ span: 16 }} 
                              >
                                <Input className="w-full z-10	"  placeholder="ค้นหา product" allowClear />
                              </Form.Item>
...
const renderOptions = (props: MasterProductContentProps): ReactElement => {
  return (
    <>
...
const renderTable = (props: MasterProductContentProps): ReactElement => {
                ...
                />
                 ...
                 <Popover content={
                    ...
                    <div  
                        ...
                        onClick={() => {
                          props.onClickToManageCarProduct({ id: row1.id , name:row1.productName});
                        }}>
                          <a>จัดการรถ</a>
                      </div>
                      ...
                      <div  
                        onClick={() => {
                          props.onClickToManageCarStock({ id: row1.id , name:row1.productName});
                        }}>
                          <a>จัดการ Stock</a>
                      </div>
                      </>
                    ...
                    <Button
                      ...
                      // onClick={() => {
                      //   props.onClickToManageCarProduct({ id: row1.id , name:"รถเช่ารายวัน"});
                      // }}
                    />
                    </Popover>
                </Space>
...
const renderModalForm = (props: MasterProductContentProps): ReactElement => {
                  ...
                  <Form form={props.form} layout="vertical" name="modal_form_layout">
                    <Row gutter={[16, 5]}>
                      ...
                      <Form.Item
                        ...
                        >
                          <Input placeholder="ชื่อ product" allowClear />
                        </Form.Item>
                      </Col>
...
// MasterProductListContainer.getLayout = (page: ReactElement) => (
//   <>
//       { page }
//   </> 
// )

export default MasterProductListContainer;